...
pub struct ClaimCreatorReward<'info> {
    ...
    #[account(
        init_if_needed,
        seeds = [b"reward_claim_creator", creator.key().as_ref()],
        bump,
        payer = creator,
...
pub fn handle_claim_creator_reward(ctx: Context<ClaimCreatorReward>, amount: u64) -> Result<()> {
    require!(amount > 0, TokenError::ZeroAmount);

    let pool = &ctx.accounts.creator_reward_pool;
    ...
    token::transfer(
        CpiContext::new_with_signer(
            ...
            Transfer {
                from: ctx.accounts.creator_reward_pool.to_account_info(),
    ...
    claim.claimant = ctx.accounts.creator.key();
