...
export class PromptpackExporter {
  private options: PortabilityExportOptions;

  constructor(options?: Partial<PortabilityExportOptions>) {
    this.options = { ...DEFAULT_PORTABILITY_EXPORT_OPTIONS, ...options };
  }

  async exportAll(directory: string, outputPath?: string): Promise<PortabilityManifest> {
  ...
  private async collectEntries(directory: string): Promise<PortabilityManifestEntry[]> {
    const promptFiles = await scanDirectoryTree(directory);
    const entries: PortabilityManifestEntry[] = [];

    for (const prompt of promptFiles) {
      try {
        const versions = this.options.includeVersions
          ? await this.collectVersions(prompt.filename, directory)
          : [];
        const license = this.options.includeLicenses
          ? await this.collectLicense(prompt.filename, directory)
          : undefined;
        const curations = this.options.includeCurations
          ? await this.collectCurations(prompt.filename, directory)
          : [];
  ...
  private async collectVersions(promptName: string, directory: string): Promise<PortabilityVersionEntry[]> {
    ...
    try {
      versionFiles = (await readdir(versionsDir))
        .filter((f) => f.endsWith(PROMPT_FILE_EXT))
        .sort();
    } catch {
      return [];
    }

    const versions: PortabilityVersionEntry[] = [];
  ...
  private async collectLicense(promptName: string, directory: string): Promise<LicenseAttachment | undefined> {
    const baseName = promptName.replace(PROMPT_FILE_EXT, "");
    const licensePath = join(directory, ".licenses", `${baseName}.license.json`);
    try {
      const content = await readFile(licensePath, "utf8");
      return JSON.parse(content) as LicenseAttachment;
    } catch {
      return undefined;
    }
  }

  private async collectCurations(promptName: string, directory: string): Promise<CurationRating[]> {
    const baseName = promptName.replace(PROMPT_FILE_EXT, "");
    const curationsDir = join(directory, ".curations", baseName);
    try {
      await stat(curationsDir);
    } catch {
      return [];
    }

    let curationFiles: string[];
    try {
      curationFiles = (await readdir(curationsDir)).filter((f) => f.endsWith(".json"));
    } catch {
      return [];
    }

    const curations: CurationRating[] = [];
    for (const cf of curationFiles) {
      try {
        const content = await readFile(join(curationsDir, cf), "utf8");
        curations.push(JSON.parse(content) as CurationRating);
      } catch {
        continue;
      }
    }
  ...
  private buildManifest(entries: PortabilityManifestEntry[], directory: string): PortabilityManifest {
    const totalVersions = entries.reduce((sum, e) => sum + e.versions.length, 0);
    const totalLicenses = entries.filter((e) => e.license).length;
    const totalCurations = entries.reduce((sum, e) => sum + e.curations.length, 0);
  ...
  private async writePromptpack(
    ...
    for (const entry of entries) {
      ...
      if (entry.license) {
        const licJson = Buffer.from(JSON.stringify(entry.license, null, 2), "utf8");
        const licName = `licenses/${entry.filename.replace(PROMPT_FILE_EXT, ".license.json")}`;
        tarParts.push(
          encodeTarHeader({
            name: licName,
            mode: 0o644,
        ...
        tarParts.push(licJson);
        tarParts.push(padToBlock(licJson));
      }

      if (entry.curations.length > 0) {
        for (let i = 0; i < entry.curations.length; i++) {
          const curJson = Buffer.from(JSON.stringify(entry.curations[i], null, 2), "utf8");
          const curName = `curations/${entry.filename.replace(PROMPT_FILE_EXT, "")}/curation_${i}.json`;
          tarParts.push(
            encodeTarHeader({
              name: curName,
              mode: 0o644,
          ...
          tarParts.push(curJson);
          tarParts.push(padToBlock(curJson));
        }
      }
    ...
    if (this.options.compress) {
      ...
      const finalPath = outputPath.endsWith(".promptpack") ? outputPath : outputPath + ".promptpack";
      await writeFile(finalPath, tarBuffer);
    }
  }

  private computeManifestChecksum(manifest: PortabilityManifest): string {
    const { checksum: _, ...rest } = manifest;
